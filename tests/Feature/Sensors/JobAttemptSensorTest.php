    ...
    public function test_it_ingests_processed_job_attempts($workCommand): void
    {
        ...
        $ingest->assertLatestWrite('job-attempt:*', [
            [
                ...
                'queue' => $this->isVapor ? 'https://sqs.us-east-1.amazonaws.com/123456789012/my-laravel-queue' : 'default',
                'status' => 'processed',
                'duration' => 2500,
                'exceptions' => 0,
                'logs' => 0,
    ...
    protected function bindLambdaEventForJob(mixed $job, int $attempts = 0): void
    {
        app()->bind(LambdaEvent::class, function () use ($job, $attempts) {
            return new LambdaEvent([
                'Records' => [
                    [
                        'messageId' => '12345678-abcd-1234-efgh-123456789012',
                        ...
                        'eventSource' => 'aws:sqs',
                        'eventSourceARN' => 'arn:aws:sqs:us-east-1:123456789012:my-laravel-queue',
                        'awsRegion' => 'us-east-1',
                    ],
                ],
            ]);
