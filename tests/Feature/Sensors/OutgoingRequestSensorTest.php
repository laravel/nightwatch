...
it('ingests outgoing requests', function () {
    ...
    $ingest->assertLatestWrite('outgoing_request:*', [
        [
            'v' => 1,
            't' => 'outgoing_request',
...
it('captures the request / response size bytes from the content-length header', function () {
    ...
    $response->assertOk();
    $ingest->assertWrittenTimes(1);
    $ingest->assertLatestWrite('outgoing_request:0.request_size', 9876);
    $ingest->assertLatestWrite('outgoing_request:0.response_size', 5432);
});
...
it('does not read the stream into memory to determine the size of the response', function () {
    ...
    $response->assertOk();
    $ingest->assertWrittenTimes(1);
    $ingest->assertLatestWrite('outgoing_request:0.request_size', null);
    $ingest->assertLatestWrite('outgoing_request:0.response_size', null);
});
...
it('captures the port when specified', function () {
    ...
    $response->assertOk();
    $ingest->assertWrittenTimes(1);
    $ingest->assertLatestWrite('outgoing_request:0.port', '4321');
});
...
it('captures the default port for insecure requests when not specified', function () {
    ...
    $response->assertOk();
    $ingest->assertWrittenTimes(1);
    $ingest->assertLatestWrite('outgoing_request:0.port', '80');
});
...
it('captures the default port for secure requests when not specified', function () {
    ...
    $response->assertOk();
    $ingest->assertWrittenTimes(1);
    $ingest->assertLatestWrite('outgoing_request:0.port', '443');
});
